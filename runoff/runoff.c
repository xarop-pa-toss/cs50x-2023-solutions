...
void tabulate(void)
{
    ...
    // Add up first non-eliminated candidate.votes
    for (int i = 0; i < voter_count; i++)
    {
        for (int k = 0; k < candidate_count; k++)
        {
...
{
    // TODO
    int bottomVotes = candidates[0].votes;

    for (int i = 0; i < candidate_count; i++)
    {
        if (!candidates[i].eliminated && candidates[i].votes < bottomVotes)
        {
            bottomVotes = candidates[i].votes;
        }
    }
